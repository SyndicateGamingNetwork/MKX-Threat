...
/**
 *	Modified population industry, for mkxthreat stations and planets, since they're operated by AIs
 */

public class Mkxthreatfragmentcore extends BaseIndustry implements MarketImmigrationModifier {
	...
	public static float AUTO_STABILITY_BONUS = 10f;
...
//		}
	...
	public static float getAccessibilityBonus(int marketSize) {
		if (marketSize <= 4) return 0.6f;
		if (marketSize == 5) return 0.7f;
		if (marketSize == 6) return 0.8f;
		if (marketSize == 7) return 0.9f;
		if (marketSize == 8) return 1.0f;
		if (marketSize == 9) return 1.1f;
		if (marketSize == 10) return 1.2f;
		return 0.3f;
	}
...
//		}
	...
	protected String getDescriptionOverride() {
		if (market.getSize() <= 4) {
			return "Being entirely automated, this station has no population, instead replaced by an extensive core of fragments";
		}
